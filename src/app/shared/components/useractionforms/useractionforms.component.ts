...
export class UseractionformsComponent implements OnInit {
  ...
  ngOnInit() {    
    this.selectedContractor = this.userActionForms.get('contractor').valueChanges.pipe(debounceTime(1000),
  ...
  contractorSelected(selected: any): void {
    ...
    this.userActionForms.get('contractor').setValue(selected ? fullName + selected?.reference : undefined);
    this.userActionForms.get('contractorId').setValue(selected ? selected?.entityId : undefined);
    this.resultsAvailable = false;
    if (this.isContractorSearch) {
      ...
      if (!selected?.isAgentContractorApproved) {
        this.userActionForms.controls['contractor'].setErrors({ invalidContractor: true, message: 'An Agreement with the Contractor doesn’t exist' });
      }

      if (selected?.supplierLiabilityExpiryDate === null || selected?.supplierLiabilityExpiryDate < currentDate) {
        this.userActionForms.controls['contractor'].setErrors({ invalidContractor: true, message: 'Supplier liability insurance date is not active' });
      }
    }
  }
